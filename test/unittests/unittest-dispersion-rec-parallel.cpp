...
std::tuple<Real, helpme::Matrix<Real>, helpme::Matrix<Real>> runTest(int nx, int ny, int nz, CalcType type) {
    ...
    if (serialRun) {
        pme->setup(6, kappa, splineOrder, gridX, gridY, gridZ, scaleFactor, numThreads);
    } else {
        pme->setupParallel(6, kappa, splineOrder, gridX, gridY, gridZ, scaleFactor, 1, MPI_COMM_WORLD,
    ...
    switch (type) {
        case E:
            nodeEnergy = pme->computeERec(0, c6s, coords);
            break;
        case EF:
            nodeEnergy = pme->computeEFRec(0, c6s, coords, nodeForces);
            break;
        case EFV:
            nodeEnergy = pme->computeEFVRec(0, c6s, coords, nodeForces, nodeVirial);
            break;
        default:
