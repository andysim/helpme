...
std::tuple<Real, helpme::Matrix<Real>, helpme::Matrix<Real>> coulombKappaSweepHelper(Real kappa, CalcType type) {
    ...
    switch (type) {
        case E:
            energy = pme.computeEAll(includedPairs, excludedPairs, 0, charges, coords);
            break;
        case EF:
            energy = pme.computeEFAll(includedPairs, excludedPairs, 0, charges, coords, forces);
            break;
        case EFV:
            energy = pme.computeEFVAll(includedPairs, excludedPairs, 0, charges, coords, forces, virial);
            break;
        default:
...
TEST_CASE(
    ...
    SECTION("EFV routine tests") {
        ...
        auto EFV25 = coulombKappaSweepHelper<double>(0.25, EFV);
        auto energy25 = std::get<0>(EFV25);
        auto forces25 = std::get<1>(EFV25);
        auto virial25 = std::get<2>(EFV25);

        auto EFV35 = coulombKappaSweepHelper<double>(0.35, EFV);
        ...
        auto EFV45 = coulombKappaSweepHelper<double>(0.45, EFV);
    ...
    SECTION("EF routine tests") {
        ...
        auto EF25 = coulombKappaSweepHelper<double>(0.25, EF);
        auto energy25 = std::get<0>(EF25);
        auto forces25 = std::get<1>(EF25);

        auto EF35 = coulombKappaSweepHelper<double>(0.35, EF);
        auto energy35 = std::get<0>(EF35);
        auto forces35 = std::get<1>(EF35);
        REQUIRE(energy25 == Approx(energy35).margin(TOL));
        REQUIRE(forces25.almostEquals(forces35, TOL));

        auto EF45 = coulombKappaSweepHelper<double>(0.45, EF);
    ...
    SECTION("E routine tests") {
        ...
        auto E25 = coulombKappaSweepHelper<double>(0.25, E);
        auto energy25 = std::get<0>(E25);

        auto E35 = coulombKappaSweepHelper<double>(0.35, E);
        auto energy35 = std::get<0>(E35);
        REQUIRE(energy25 == Approx(energy35).margin(TOL));

        auto E45 = coulombKappaSweepHelper<double>(0.45, E);
