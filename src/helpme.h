...
struct SplineCache {
    ...
    /// The angular momentum of the parameters in the cache.
    size_t parameterAngMom_ = 0;
    /// The derivative level this cache supports.
    size_t derivativeLevel_ = 0;
};
...
#pragma omp parallel num_threads(nThreads_)
     ...
     *              Lx  = L - Ly - Lz
     * \endcode
     * \param derivativeLevel the order of potential derivative needed when probing the grid.
    ...
    void filterAtomsAndBuildSplineCache(int parameterAngMom, const RealMat &parameters, int derivativeLevel,
        ...
        size_t splineDerivativeLevel = parameterAngMom + derivativeLevel;
        ...
        for (size_t atom = nAtoms; atom < paddedNumAtoms; ++atom) newSplineCache_.atomList_.push_back(0);

        newSplineCache_.paddedNumAtoms_ = paddedNumAtoms;
        newSplineCache_.splineOrder_ = splineOrder_;
        newSplineCache_.parameterAngMom_ = parameterAngMom;
        newSplineCache_.derivativeLevel_ = derivativeLevel;
        ...
        size_t nParameterComponents(nCartesian(std::abs(parameterAngMom)));
        if (parameterAngMom < 0) nParameterComponents -= nCartesian(std::abs(parameterAngMom) - 1);
        newSplineCache_.fractionalCoordinateParameters_.resize(paddedNumAtoms * nParameterComponents);
        std::fill(newSplineCache_.fractionalCoordinateParameters_.begin(),
                  newSplineCache_.fractionalCoordinateParameters_.end(), 0);
...
#pragma omp parallel for reduction(+ : energy) num_threads(nThreads_)
    ...
    void computeForces(const Real *potentialGrid, int parameterAngMom, const RealMat &parameters, RealMat &forces) {
        ...
        const size_t *atomList = newSplineCache_.atomList_.data();
        const size_t splineOrderCubed = splineOrder_ * splineOrder_ * splineOrder_;
        const size_t angMomStart = 1;
        Real *paramPtr = newSplineCache_.fractionalCoordinateParameters_.data();
        const size_t paddedNumAtoms = newSplineCache_.paddedNumAtoms_;
#pragma omp parallel num_threads(nThreads_)
        {
#ifdef _OPENMP
            int thread = omp_get_thread_num();
...
#pragma omp parallel for num_threads(nThreads_)
            for (size_t atomChunk = 0; atomChunk < paddedNumAtoms; atomChunk += REALVECLEN) {
                ...
                REALVEC Fy = 0.0f;
                REALVEC Fz = 0.0f;
                for (int component = 0; component < nParameterComponents; ++component) {
                    REALVEC fractionalParams =
                        MAKEVEC(paramPtr[atomChunk * nParameterComponents + component * REALVECLEN]);
                    const auto &quanta = angMomIterator_[component];
                    size_t xDeriv = quanta[0];
                    size_t yDeriv = quanta[1];
                    size_t zDeriv = quanta[2];
                    Fx += fractionalParams * fractionalPotentials[cartAddress(xDeriv + 1, yDeriv, zDeriv)];
...
#pragma omp parallel for num_threads(nThreads_)
    ...
    Real computeERec(int parameterAngMom, const RealMat &parameters, const RealMat &coordinates) {
        sanityChecks(parameterAngMom, parameters, coordinates);
        filterAtomsAndBuildSplineCache(parameterAngMom, parameters, 0, coordinates);
    ...
    Real computeEFRec(int parameterAngMom, const RealMat &parameters, const RealMat &coordinates, RealMat &forces) {
        ...
        filterAtomsAndBuildSplineCache(parameterAngMom, parameters, 1, coordinates);
